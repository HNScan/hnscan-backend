...
class HnscanDB extends EventEmitter {
  ...
  async setDifficulty(time, difficulty) {
    let date = Math.floor(time / (3600 * 24));

    if (this.chartDataCurrentDate === 0) {
      //initialization
      this.chartDataCurrentDate = date;
      this.chartDataCurrent.push(difficulty);
    } else if (date > this.chartDataCurrentDate) {
      //average out the the current tick data.
      let sum = this.chartDataCurrent.reduce(
        (previous, current) => (current += previous)
      );
      let avg = sum / this.chartDataCurrent.length;

      await this.db.put(layout.d.encode(date * 3600 * 24), fromDouble(avg));
    ...
    //While technically not a u32, I think we will hit some other limits before difficulty breaks that space.
    // await this.db.put(layout.d.encode(time), fromDouble(difficulty));
  }
  ...
  async getDifficultySeries(startTime, endTime) {
    ...
    await iter.each(async (key, rawValue) => {
      ...
      // let tick = {
      //   time: time,
      //   difficulty: toDouble(rawValue)
      // };

      //Remove this. Need to fix lineChart in UCL first tho.
      let tick = {
        date: time * 1000,
        visits: toDouble(rawValue)
      };
...
function fromU64(num) {
  const data = Buffer.allocUnsafe(8);
  data.writeBigUInt64LE(num, 0, true);
  return data;
}

function toU64(buf) {
  const num = buf.readBigUInt64LE(0, true);
  return num;
}

function fromDouble(num) {
  const data = Buffer.allocUnsafe(8);
  data.writeDoubleLE(num, 0, true);
  return data;
}

function toDouble(buf) {
  const num = buf.readDoubleLE(0, true);
  return num;
}
