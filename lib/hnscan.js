...
class Hnscan extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.nameCount = 0;
    this.nameCountBlock = 0;
  }

  //Calls all of the resource heavy API calls, to cache them for faster response times.
  //@todo make sure to add this to unindex block when that is implemented.
  async memorize(height) {
    this.logger.info("HNScan memorizing values...");
    await this.getNameCount(height);
  }

  async getNameCount(height) {
    if (height === this.nameCountBlock) {
      return this.nameCount;
    }

    let txn = this.chain.db.txn;
  ...
  async getTransaction(hash) {
    ...
    for (const input of tx.inputs) {
      ...
      if (!input.coin) {
        if (index === 0) {
          json.value = consensus.getReward(
            tx.height,
  ...
  //@todo build out the name over these function calls.
  async getName(name) {
    ...
    data.hash = nameHash.toString("hex");
    data.release = start;
    //@todo perfect, go off of this.
    data.state = info ? info.state : "INACTIVE";

    //Advanced
    data.height = start;
    data.value = info ? info.value : 0;
    data.renewal = info ? info.renewal : 0;
    data.renewals = info ? info.renewals : 0;
    //@todo
    data.weak = info ? info.weak : false;
    data.transfer = info ? info.transfer : 0;
    data.highest = info ? info.highest : 0;
    data.revoked = info ? info.revoked : 0;
    data.blocksUntil = info ? Object.values(info.stats)[2] : null;
